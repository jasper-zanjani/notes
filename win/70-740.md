...
- [Tasks](#tasks)
    ...
    - [Failover cluster lab](#failover-cluster-lab)
          ...
          - [Configure iSCSI Server Shared Volumes](#configure-iscsi-server-shared-volumes)
      ...
      - [Scale-Out Fileserver lab](#scale-out-fileserver-lab)
          ...
          - [Configure iSCSI Server Shared Volumes](#configure-iscsi-server-shared-volumes-1)
        - [Configure iSCSI initiators to iSCSI Target Server](#configure-iscsi-initiators-to-iscsi-target-server-1)
...
```powershell
Set-Disk -Number 5,6 -IsOffline $false
```
Then create new simple volumes from the unallocated space, mounting them to **Q** (for the 500 MB "QuorumHD") and **V** (for the 30 GB cluster shared volume).
```powershell
...
```
Repeat these steps on **PLABSA01** (the disk numbers may differ).
```powershell
New-Volume -DiskNumber 4 -DriveLetter Q -FileSystem NTFS -FriendlyName 'Quorum'
New-Volume -DiskNumber 5 -DriveLetter V -FileSystem NTFS -FriendlyName 'CSV'
```
...
##### Configure iSCSI initiators to iSCSI Target Server
```powershell
New-IscsiServerTarget -TargetName PLABDC01 -InitiatorId @("IPAddress:192.168.0.20","IPAddress:192.168.0.40","IPAddress:192.168.1.20","IPAddress:192.168.1.40")
```
Start iSCSI initiator service and open firewall
```powershell
# PLABDM01
Start-Service MSiSCSI
Get-NetFirewallServiceFilter -Service msiscsi | Get-NetFirewallRule | Enable-NetFirewallRule
```
Configure an iSCSI Target Portal (won't work if you define DNS names of initiators in `New-iSCSIServerTarget` command above)
```powershell
New-iSCSITargetPortal -TargetPortalAddress 'plabdc01'
```
The commands below still need some work ("target name is not found or is marked as hidden from login")
```powershell
$t = Get-iSCSITarget
