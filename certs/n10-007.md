...
802.3ab   | Gigabit Ethernet on Cat 5+ [[4](#sources): 101]
802.3ae   | Gigabit Ethernet over fiber and coax [[4](#sources): 101]
802.3af   | PoE
...
## Network principles
#### Topology
Wired topologies [[1](#sources): 2-6]
...
Wireless topologies [[1](#sources): 7-11]
...
#### IoT
**Internet of Things (IoT)** [[1](#sources): 20-23]
...
#### WLAN
**Wireless LAN (WLAN)** [[1](#sources): 016]
...
- **multiuser multiple input multiple output (MU-MIMO)** set of advanced MIMO technologies included with 802.11ac that improve wireless throughput [[1](#sources): 025-6]
...
Wireless encryption
- **Wired equivalent privacy (WEP)** early wireless encryption technology [[1](#sources): 373-4, 506]
- **Wi-Fi protected access (WPA)** wireless encryption technology created to address the problems of WEP [[1](#sources): 373-4, 507]
- **Temporal key integrity protocol (TKIP)** inferior encryption algorithm used by WPA [[1](#sources): 374, 502]
- **Counter-mode Cipher Block Chaining Message Authentication Code Protocol (CCMP)** encryption algorithm used by WPA2,l using 128-bit AES encryption with a 48-bit initialization vector [[1](#sources): 374]
...
- **IP Address Management (IPAM)** means of planning, tracking, and managing IP addresses usedin a network [[1](#sources): 076]
...
#### Tunneling
- **4to6** tunneling technology that allows IPv4 packets to be transmitted over an IPv6 network [[1](#sources): 100]
- **6to4** tunneling technology that allows IPv6 packets to be transmitted over an IPv4 network [[1](#sources): 100]
- **Miredo** an implementation of [Teredo](#glossary) that is designed to allow full IPv6 connectivity to systems that are strictly IPv4-based [[1](#sources): 100]
- **Teredo** tunneling technology that gives full IPv6 connectivity for IPv6-capable hosts that are not directly connected to an IPv6 network [[1](#sources): 100]
...
Switching methods [[1](#sources): 108-110]
- **Packet switching**: also known as independent routing, messages are broken down into smaller pieces called **packets**
  ...
  - **Datagram** packet switching: packets are independently sent
    - **independent routing** allowing **packets** travel different routes to avoid high-traffic areas [[1](#sources): 108]
- **Circuit** switching: requires a dedicated physical connection between sending and receiving devices
- **Distributed** switching: architecture in which multiple processor-controlled switching units characterized by a hierarchy of switches with a centralized host switch  [[1](#sources): 108]

#### VLAN
- **trunking** also known as **link aggregation**, use of multiple network cables or ports in parallel to increase the link speed beyond the limits of any single cable or port [[1](#sources): 126]
  - **trunk port** port that is assigned to carry traffic for a specific switch, as opposed to an **access port** [[1](#sources): 120]
- **port aggregation** combining of multiple ports on a switch [[1](#sources): 120]
  - **NIC teaming** process of combining multiple network cards for performance and redundancy [[1](#sources): 314]
- **tagging** sending out a packet with a tag number that indicates its VLAN tag number
- **VLAN pooling** multiple interfaces are treated as a single entity for load balancing purposes [[1](#sources): 150]
- **native VLAN** the only VLAN that is not tagged in a trunk, typically port 1 [[1](#sources): 390]
- **port binding** determines whether and how a port is bound [[1](#sources): 120]

VLAN protocols
- **VLAN Trunk Protocol (VTP)** Cisco proprietary protocol used to reduce administration in the switched network [[1](#sources): 120]
- **Link Aggregation Control Protocol (LACP)** common aggregation protocol that allows multiple physical ports to be bound together [[1](#sources): 121]
...
Adjusting the functionality of switch interfaces: [[1](#sources): 125, 363]
- **Tag or untag VLANs**
...
- **Static routing** manually entered routes
  - **routing table** chart used by the TCP/IP stack to determine the best possible path to send data [[1](#sources): 106]
- **Dynamic routing** involves routers communicating known routes to all directly attached routers. Communication between distance-vector routers is known as **hops**: each router represents one hop. There are **two** types of routing protocols used:
  - **Distance-vector protocols** operate by having each router send updates about all the other routers it knows about to directly connected routers, with updates automatically sent every 30 or 60 seconds. Routers can be configured to send **triggered updates** when a change is detected. The process by which routers learn of updates to network topology is called **convergence**.
    ...
    - **Enhanced Interior Gateway Routing Protocol (EIGRP)** enables routers to exchange information more efficiently than RIP, using neighbors to determine routing information. Uses **Diffusing Update Algorithm (DUAL)** to determine best route to a destination.
      - **Diffusing Update Algorithm (DUAL)** used by **EIGRP** [[1](#sources): 112]
  ...
  - **Hybrid Routing Protocols**
    - **Border Gateway Protocol (BGP)** can be used between gateway hosts on the Internet. BGP supports the use of **autonomous system numbers (ASN)**, globally unique numbers used by connected groups of IP networks that share the same routing policy.
      - **autonomous system number (ASN)** globally unique numbers used by connected groups of IP networks that share the same routing policy [[1](#sources): 115]
...
- **time to live (TTL)** value assigned to a packet of data to prevent it from moving around the network indefinitely; every hop subtracts one from this number, and when the packet's TTL reaches zero, a message is sent back to the sender [[1](#sources): 433]
...
## Technologies
#### WAN
- **channel service unit/data service unit (CSU/DSU)** acts as translator between the LAN and WAN data format, with physical connections for both sets of equipment [[1](#sources): 189]

**Digital subscriber line (DSL)** [[1](#sources): 176-177]
- **Extended Digital Subscriber Line (XDSL)** label for all the variations of DSL available lumped together [[1](#sources): 176-7]
...
ISDN
- **Basic Rate Interface (BRI)** **ISDN** type containing two 64kbps B channels and one 16 kbps D channel [[1](#sources): 161]
- **Primary Rate Interface (PRI)** **ISDN** type built on a T1 or E1 circuit, containing 23 B channels and one 64 kbps D channel [[1](#sources): 161]

### Cabling
**Cross-connect** the point where the cables running throughout the network meet and are connected [[1](#sources): 216-220]
...
Twisted pair cable types [[1](#sources): 212]
...
#### Fiber
Fiber-optic cable:
- **Single-mode fiber (SMF)** single direct beam of light, which allows for greater distances and transfer speeds [[1](#sources): 202]
- **Multimode fiber (MMF)** many beams of light travel through the cable, bouncing off the cable walls and resulting in signal attenuation [[1](#sources): 202; [4](#sources); 63]
...
Other media:
- **Hybrid fiber-coaxial (HFC)** mixed fiber-optic and coaxial cabling network that uses fiber in the trunk portions to provide high speeds and coaxial to distribute it to the homes [[4](#sources): 501]

**Fiber transceiver** bidirectional devices capable of operating in duplex mode, used to connect a Gigabit Ethernet router to a fiber network [[1](#sources): 209-210]
- **gigabit interface converter (GBIC)** 1 Gbps switch port module which can be quickly inserted into or removed from a modular Ethernet switch [[1](#sources): 209, 477]
- **small form factor pluggable (SFP)** small optical transceivers [[1](#sources): 209, 497]
...
## Protocols
Email protocols [[1](#sources): 47-49]
...
- **cache** table on the local system that stores mappings between data link layer addresses and network layer addresses [[1](#sources): 061]
...
- **text record (TXT)** originally created to carry human-readable text in a DNS record, but today more commonly holds machine-readable data [[1](#sources): 71]
  - **DomainKeys identified mail (DKIM)** machine-readable data that may appear in a DNS TXT record [[1](#sources): 071]
  - **sender policy framework (SPF)** machine-readable data that may appear in a DNS TXT record [[1](#sources): 071]

#### NetBIOS
**Network Basic Input/Output System (NetBIOS)** software application that enables different applications to communicate between computers on a LAN [[1](#sources): 072]\
...
## Cloud
Cloud service models [[1](#sources): 267-270]
...
Network test types [[1](#sources): 329]
...
Primary event logs in Windows [[1](#sources): 331]
...
## Security
IDS/IPS
- **intrusion detection system (IDS)** detects the presence of an attack, and then logs that information [[1](#sources): 150, 480]
- **intrustion prevention system (IPS)** network device that continually scans the network, looking for inappropriate activity [[1](#sources): 150, 483]
- **behavior-based** also "anomaly-detection IDS (AD-IDS)", looks for variations in metrics [[1](#sources): 151; [4](#sources): 468]
- **signature-based** also "misuse-detection/prevention system" (MD-IDS/MD-IPS), focuses on evaluating attacks based on attack signatures and audit trails using an extensive database to determine the signature [[1](#sources): 151; [4](#sources): 468]
- **network intrusion detection/prevention system (NIDS/NIPS)** examines all network traffic to and from network systems; may be connected to a hub or switch if it is a hardware device [[1](#sources): 151]
- **host intrusion detection/prevention system (HIDS/HIPS)** applications such as spyware or virus applications that are installed on individual network systems, and create logs locally [[1](#sources)]

#### Authorization
**Authentication, Authorization, and Accounting (AAA)** primary method for access control, often using **RADIUS**, **TACACS+**, or **Kerberos** to accomplish integrated security [[1](#sources): 126]
- **Authentication** process of determining whether someone is authorized to use the network [[1](#sources): 462]]
- **Authorization** identifying the resources a user can access after authentication [[1](#sources): 462]]
- **Accounting** tracking methods used to identify who uses the network and what they do on it [[1](#sources): 462]]
- **Authentication, Authorization, Accounting, and Auditing (AAAA)** conceptual model for managing network security through one central location [[1](#sources): 462; [4](#sources): 403]
- **Remote Authentication Dial-In User Service (RADIUS)** protocol that enables a single server to become responsible for all remote-access authentication, authorization, and auditing services [[1](#sources): 363]
- **Terminal Access Control Access Control System (TACACS)** security protocol designed to provide centralized validation of users who are attempting to gain access to a router or NAS [[1](#sources): 363]
- **Kerberos** security system that establishes a user's identity when they first log on to a system [[1](#sources): 364-6, [4](#sources): 402]
  - **ticket** security tokens issued to clients who successfully authenticate to a server under Kerberos [[1](#sources): 366, [4](#sources): 402]
- **Local authentication subsystem (LASS)** authenticates users with locally stored data when the central authentication server is unavailable [[1](#sources): 367]
...
#### DoS attacks
**Denial of Service attack (DoS)** target system is overwhelmed with requests for service, keeping it from servicing legitimate requests [[1](#sources): 378, 473]
- **distributed DoS (DDoS)** DoS attack that utilizes more than one computer in the attack [[1](#sources): 378, 471]
- **distributed reflective DoS (DRDoS)** also "amplification attack", targeting public UDP servers like those supporting NTP, DNS, NetBIOS, SNMP, etc [[1](#sources): 380]
- **permanent denial of service (PDoS)** continues for more than a short period of time and requires changes in routing, IP addresses, or other configuration to counteract [[1](#sources): 379, 491-492]
...
#### Pentesting
- **Black Box** pentester has absolutely no knowledge of system and is functioning just as an outside attacker [[1](#sources): 395]
- **Gray Box** pentester has some knowledge of system [[1](#sources): 395]
- **White Box** pentester has significant knowledge of the system and simulates an attack from an insider [[1](#sources): 395]
...
##### Dulaney
"Exam Cram: CompTIA Network+ N10-007". **Dulaney**, Emmett. 
