...
## Topics
- [x] [**1**](#installation) Install Windows Servers in host and compute environments
  - [x] [1.1][1.1] Install, upgrade, and migrate servers and workloads
  - [x] [1.2][1.2] Install and configure Nano Server
  - [x] [1.3][1.3] Create, manage, and maintain images for deployment
- [ ] [**2**](#storage) Implement storage solutions
  - [ ] [2.1][2.1] Configure disks and volumes
  - [ ] [2.2][2.2] Implement server storage
  - [ ] [2.3][2.3] Implement data deduplication
- [ ] [**3**](#hyper-v) Implement Hyper-V
  - [x] [3.1][3.1] Install and configure Hyper-V
  - [x] [3.2][3.2] Configure virtual machine (VM) settings
  - [ ] [3.3][3.3] Configure Hyper-V storage
  - [x] [3.4][3.4] Configure Hyper-V networking
- [ ] [**4**](#containers) Implement Windows containers
  - [ ] [4.1][4.1] Deploy Windows containers
  - [ ] [4.2][4.2] Manage Windows containers
- [ ] [**5**](#clusters) Implement high availability
  - [ ] [5.1][5.1] Implement high availability and disaster recovery options in Hyper-V
  - [ ] [5.2][5.2] Implement failover clustering
  - [ ] [5.3][5.3] Implement Storage Spaces Direct
  - [ ] [5.4][5.4] Manage failover clustering
  - [ ] [5.5][5.5] Manage VM movement in clustered nodes
  - [ ] [5.6][5.6] Implement Network Load Balancing (NLB)
- [ ] **6** Maintain and monitor server environments
  - [ ] [6.1][6.1] Maintain server installations
  - [ ] [6.2][6.2] Monitor server installations
...
These packages are provided as tarballs or ISO images, and must be loaded directly into the running guest.
FreeBSD includes full support for FreeBSD Integration Services (BIS) since version 10.
...
To configure a simple failovers solution using [Hyper-V Replica][Hyper-V Replica], configure the server as a [replica][replica server], specify authentication type (HTTP via Kerberos or HTTPS), and specify authorization option. <sup>[pwsh][Set-VMReplicationServer]
Finally enable appropriate firewall rule, depending on authentication type chosen. <sup>[pwsh][Enable-NetFirewallRule]</sup>
On the primary server that will be replicated, you have to configure options through the Enable Replication wizard.
...
[Shared Nothing Live Migration][Shared Nothing Live Migration] requires that source and destinatino VMs be members of the same (or trusted) domain, and source and domain servers must be running the same processor family (Intel or AMD) and linked by an Ethernet network running a minimum of 1 Gbps. 
