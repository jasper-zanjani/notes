...
Virtual disks can be [thinly provisioned][thinly provisioned], if the drive is expected to grow in the future.
...
[Storage Replica][Storage Replica] supports synchronous and asynchronous replication (one-way only) to function between two standalone servers, between clusters, and between storage devices within an asymmetric cluster.
Storage Replica improves on DFS Replication, which is exclusively asynchronous and file-based, by using SMBv3.
#### Data deduplication
