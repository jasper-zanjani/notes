...
- Disk and GPU passthrough, and other PCI-x devices, with [DDA][DDA]
...
VMs created by older versions of Hyper-V can be [updated][Update-VMVersion], but once upgraded thaey may no longer run on a host of a previous version.

Importing an exported VM can be done in three ways: 
"Registering" the VM means the exported files are left as-is and the VM's ID is maintained; 
"Restoring" it copies the exported files to the host's default locations or ones that are otherwise specified while also maintaining the ID; 
finally, "Copying" it copies the exported files and generates a new ID.
