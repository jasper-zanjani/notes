...
To configure a simple **one-way** failover solution using [**Hyper-V Replica**][Hyper-V Replica], configure the **destination** VM as a [**replica server**][replica server]. <sup>[pwsh][Set-VMReplicationServer] [lab](#hyper-v-replica-lab)</sup>
On the VM that will be replicated, configure options through the **Enable Replication wizard**.
To use Hyper-V Replica as a failover server, configure both VMs as replica servers.

Migrations can take place one of three methods:
- [**Live Migration**][Live Migration] does not move a VM's data files - it moves only the system state and live memory contents - and requires that VHD files be placed on shared storage and both hosts have appropriate permissions to access said storage.
...
The predecessor to Live Migration was called **Quick Migration**.
...
[Failover clusters][failover cluster] are created using `New-Cluster`.
The computers and VMs that compose them are called **nodes** and typically possess a secondary network adapter, which is used for cluster communications and must be configured with a static IP and a preferred DNS server.

Before Windows Server 2016, all cluster nodes had to belong to the same domain, but now this is but one type of possible cluster called a **single-domain** cluster.
...
In the absence of Active Directory entirely, the cluster's network name must be registered with DNS to form its **administrative access point**.
Additionally, on a workgroup cluster the same local administrator account must be created on every node.
As above, the cluster can be created with Powershell by specifying the `AdministrativeAccessPoint` named parameter.
