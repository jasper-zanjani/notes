...
3. Given a scenario, implement secure systems design <br> [SEC](sec)
4. Explain the importance of secure staging deployment concepts
5. Explain the security implications of embedded systems <br> [RTOS](#embedded-systems)
6. Summarize secure application development and deployment concepts
7. Summarize cloud and virtualization concepts <br> [**Cloud deployment models**](n10-007.md#cloud)
...
4. Given a scenario, follow incident response procedures <br> **Incident response process**: [Preparation][Preparation] [Identification][Identification] [Containment][Containment] [Eradication][Eradication] [Recovery][Recovery] [Lessons learned][Lessons learned]
...
2. Explain cryptography algorithms and their basic characteristics <br> [**Symmetric ciphers**](#symmetric-ciphers) AES DES 3DES RC* Twofish Blowfish &bull; [**Asymmetric ciphers**](#asymmetric-ciphers) RSA DSA D-H ECC PGP GPG &bull; [**Ephemeral protocols**](#forward-secrecy) DHE ECDHE &bull; Block cipher **[modes of operation][mode of operation]**: [CBC][CBC] [ECB][ECB] [GCM][GCM] [CFB][CFB] [CTR][CTR] &bull; [**Hashes**](#hashes) MD5 SHA RIPEMD HMAC &bull; [**Key stretching**](#key-stretching) BCrypt PBKDF2 
...
### Attacks
#### Malware attacks
**Malware attacks** include [adware][adware], [scareware][scareware] [spyware][spyware], [backdoors][backdoor], [botnets][botnet], [keyloggers][keylogger], [rootkits][rootkit], [spyware][spyware], [trojans][trojan] (including [remote access trojans][RAT]), [viruses][virus], and [worms][worm]

#### Social engineering
[**Phishing**](#social-engineering "getting personal details") attacks include [spear phishing][Spear phishing], [whaling][Whaling], and [vishing][Vishing]

[Spear phishing]: #social-engineering 'Spear phishing&#10;Targeting a group of people'
[Vishing]: #social-engineering 'Vishing&#10;Leaving a voicemail or using a VoIP phone'
[Whaling]: #social-engineering 'Whaling&#10;Targeting high-level executives'
...
[spear phishing]:       #attacks "\"spear phishing\"&#10; targeting a group of people"
[whaling]:              #attacks "\"whaling\"&#10; targeting high-level executives"

#### Social engineering


- **Cross-Site Scripting attack (XSS)** when a user injects malicious code into another user's browser, using both HTML and JavaScript
- **Privilege escalation** attacker grants additional rights to a standard user account
- **Man in the Browser (MITB)** Trojan is used to intercept and manipulate calls between the browser and its security mechanisms
...
Network devices include [firewalls][firewall], [routers][router], [switches][switch], load balancer, proxy server, bastion host, web security gateway, VPN concentrator, protocol analyzer, UTM, Spam filter, URL filter, content filter, malware inspection filter, web application firewall
...
- **Bastion host** exposed server that provides public access to a critical service (i.e. proxy, web, or email server), configured to isolate it from an organization's internal network
- **Web security gateway** offers a single point of policy control and management for web-based content access
- **VPN concentrator** allows multiple external users to access internal network resources securely
  - **Scalable Encryption Processing modules (SEP)** component of Cisco VPN concentrators
...
[firewall]: #devices "\"firewall\"&#10; networking device that controls access to a network and designed to protect data and resources from an outside threat"
[router]: #devices "\"router\"&#10; create, add, or divid networks or network segments at the L3 layer"
[switch]: #devices "\"switch\"&#10; forwards data to destination device at the L2 layer via twisted-pair cabling"
...
Firewall rules [Barrett][Barrett]
...
[SEC]: #sec "\"Self-Encrypting Drive (SEC)\"&#10;performs encryption using onboard hardware, so the drive content is always encrypted&#10;Eckert, Jason. _Linux+ Guide to Linux Certification_. Course Technology, 2012"
[SPF]: # "\"Sender Policy Framework (SPF)\"&#10;machine-readable data that may appear in a DNS TXT record&#10;Dulaney, Emmett. _Exam Cram: CompTIA Network+ N10-007_: 71"
...
#### Nonrepudiation
**Nonrepudiation** makes it impossible to refute the origin of data
...
- **Rivest, Shamir, Adelman (RSA)** asymmetric-key cryptographic algorithm that can use a key of any length and which was released into the public domain in 2000; works by multiplying two large prime numbers and derives a set of numbers through other operations
  - **Digital Signature Algorithm (DSA)** developed by the NSA in 1991 as an alternative to RSA which is faster in signing but slower in verifying, thus preferable if there are performance issues on the client
- **Diffie-Hellman key exchange (D-H)** also "exponential key agreement", an early asymmetric-key cryptographic algorithm, vulnerable to man-in-the-middle attacks.

It was the first prime-number algorithm and patented in 1977. Requires sharing of an arbitrary number which is multiplied by a secret number selected by each party. Both parties exchange the product, which is then multiplied by their own secret number which should produce the same result.
- **Elliptic curve cryptography (ECC)** asymmetric key encryption algorithm based on complex mathematical structures; it uses smaller key sizes than traditional public-key cryptosystems and is therefore faster and less resource-intensive than traditional system
- **El Gamal encryption algorithm** asymmetric key encryption algorithm that improved upon D-H by incorporating encryption on key exchange elements

#### Symmetric algorithms
- **Advanced Encryption Standard (AES)** also "Rijndael" after the name of the original cipher upon which AES is based, symmetric-key cyptographic algorithm using a block size of 128 bits and key sizes of 128, 192, or 256 bits
- **Data Encryption Standard (DES)** obsolete symmetric-key block cipher cryptographic algorithm superceded by AES; uses a random key selected from a large number of shared keys; most forms are export-controlled; Feistel cipher with a 64-bit block size and 56-bit key; adopted for use by NIST in 1977
- **Triple Data Encryption Standard (3DES)** symmetric key encryption algorithm that uses DES three times with three different keys, providing a total bit strength of 168 bits
- **Rivest Cipher (RC2, RC5, RC6)** family of interrelated symmetric-key block ciphers
  - **RC4** flawed stream cipher used by WEP using a shared secret key to generate a long sequence of bytes from a generator 
- **Twofish** symmetric-key cryptographic algorithm that uses a 128-bit block size with a maximum key size of 128 bits that lost the competition to replace DES; related to the earlier Blowfish cipher
  - **Blowfish Encryption Algorithm** symmetric-key block cipher that uses a 16-round Feistel cipher with a 64-bit block and a variable key length up to 448 bits

#### Cipher modes
Block cipher **[modes of operation][mode of operation]**: [CBC][CBC] [ECB][ECB] [GCM][GCM] [CFB][CFB] [CTR][CTR]

#### Key stretching algorithms
**Key stretching** makes an existing key or password stronger by performing a function to increase the time it might otherwise take to crack it by brute force; takes the original key and feeds it through an algorithm which results in a stronger key
...
#### Hashing algorithms
Summary generated from a mathematical algorithm, commonly used as a digital fingerprint to verify the integrity of files
- **Message digest series algorithm (MD2, MD4, MD5)** series of hash algorithms created by Ronald Rivest, generating a hash of up to 128-bit strength out of any length of data
- **Secure hash algorithm (SHA, SHA-1, SHA-2, SHA-3)** hash algorithms developed by NSA; SHA-224, SHA-256, SHA-384, and SHA-512 together are known as SHA-2
...
**Message authentication code (MAC)** also "authentication tag", derived by applying a applying a message or file combined with a secret key to a cryptographic algorithm; basically an encrypted hash
- **Hash-based message authentication code (HMAC)** MAC based on cryptographic hash functions; provides additional security by adding an additional integrity check to the data being transmitted
...
#### Forward secrecy
**Perfect forward secrecy** process of destroying **session keys** after communication session is complete. A **session key** also "symmetric key", is a randomly generated key to perform both encryption and decryption during a session\
**Ephemeral key agreement protocol** provides **perfect forward secrecy**, at computational cost
- **Diffie-Hellman (DHE)** uses an ephemeral algorithm
- **Elliptic Curve Diffie-Hellman (ECDH)** variant of Diffie-Hellman key agreement protocol for ECC using an ephemeral mode of operation

#### Cipher suite
Combination of several algorithm with a name composed of identifiers concatenated together with the underscore `_`, i.e. `TLS_DHE_DSS_WITH_3DES-EDE-CBC_SHA` 
1. **Protocol**
...
**WPA2 modes**
- **WPA2-Personal (WPA2-PSK)** uses a pre-shared key (PSK) of 64 hexadecimal digits or a passphrase of 8 to 63 ASCII characters
- **WPA2-Enterprise** verifies network users through a server

WLAN encryption
- **Counter-mode Cipher Block Chaining Message Authentication Code Protocol (CCMP)** encryption algorithm used by WPA2, using 128-bit AES encryption with a 48-bit initialization vector; based on AES and superceding TKIP
- **Temporal key integrity protocol (TKIP)** inferior encryption algorithm used by WPA that was the first to introduce a dynamic, per-packet, 128-bit key
- **RC4** flawed stream cipher used by WEP using a shared secret key to generate a long sequence of bytes from a generator 
...
**Administrative controls** also "management controls", business and organizational processes and procedures and administrative controls [[^][Barrett]: 85, 170] <br>mainly written by managers to create organizational policies to reduce the risk within companies [[Neil][Neil]: 7]
...
#### Physical controls
[**Physical controls**](#physical-controls 'Also "operational controls", controls you can touch and form the outer line of defense against direct access to data') include [**Faraday cage**](#physical-controls "metal structure preventing wireless or cellular phones from working within it"), [**bollards**](#physical-controls "stop cars from ramming an entrance"), [cable locks](#physical-controls "prevent laptop theft"), [laptop safes](#physical-controls "store laptops and tablets"), [biometric locks](#physical-controls "biometric signatures like fingerprint, retina, palm, or voice scans are unique to individuals") [perimeter fences](#physical-controls "control access to the site") [burglar alarms](#physical-controls "triggers an alarm when premises are broken into when unoccupied") [lighting](#physical-controls "illuminate entrants to site at night") [security guards](#physical-controls "check identify of entrants") [mantraps](#physical-controls "turnstile devices that allow only one person in at a time") [key management](#physical-controls "departmental keys are signed out and in daily to prevent copying of keys") [proximity card](#physical-controls "contactless devices that allow bearers of a smart card access") [tokens](#physical-controls "small physical devices that provide access through proximity card readers, buttons, or numbers") [HVAC](#physical-controls "\"heating, ventilation, and air-conditioning\", keeps servers in data centers cool") and [air gapping](#physical-controls "device taken off a network to isolate it") [^][Barrett] [^][Neil]
...
- **Domain Validated certificate (DV)** X.509 certificate typically used for TLS, where the domain name of the applicant has been validated by proving some control over a DNS domain 
...
- **Open ID Connect** uses OAuth to allow users to login to a web application without needing to manage the user's account [^][Neil]
...
FTPS [implicit][implicit mode] and [explicit][explicit mode] modes
...
[physical controls](# "Also "operational controls", controls you can touch and form the outer line of defense against direct access to data &#10;e.g. Faraday cage, bollards, biometric locks")
...
[Kerberos]:               #kerberos "Symmetric-key authentication protocol developed by MIT and used in Active Directory to provide an abstracted authentication process (TCP and UDP port 88, previously ports 749 and 750)"
[mutual authentication]:  #kerberos "\"mutual authentication\"&#10;, both client and server verify that the computer with which they are communicating is the proper system"
[KDC]:                    #kerberos "\"key distribution center\", destination of client authentication details, rather than target center"
[TGS]:                    #kerberos "\"Ticket-Granting Server\"&#10; destination of TGT sent by client"
[TGT]:                    #kerberos "\"Ticket-Granting Ticket\"&#10; time-stamped session key sent by CA to client"
[ticket]:                 #kerberos "\"ticket\"&#10; security tokens issued to clients who successfully authenticate to a server under Kerberos"
[mode of operation]:      #cipher-modes "\"mode of operation\"&#10; encryption of messages with sizes different from that of the block length"
[CBC]:                    #cipher-modes "\"Cipher Block Chaining \"&#10; an initialization vector using a random public value is XORed with the plaintext prior to encryption for the first round; subsequent rounds use the ciphertext of the previous round"
[ECB]:                    #cipher-modes "\"Electronic Code Book\"&#10; simplest block cipher mode of operation where each block of plaintext is encrypted separately"
[GCM]:                    #cipher-modes "\"Galois Counter Mode\"&#10; special case of CTR that doesn't use a nonce but relies only on a counter and calculates a MAC"
[CFB]:                    #cipher-modes "\"Cipher Feedback\"&#10; an initialization vector is encrypted and the result is XORed with the plaintext to create the ciphertext of the block; first block is a random value and other blocks use the previous block's ciphertext"
[CTR]:                    #cipher-modes "\"Counter mode\"&#10; every encryption operation is separate but uses a nonce and a counter; plaintext is XORed with the output of the encryption operation to produce ciphertext"
[adware]:                 #attacks "\"adware\"&#10; displays popups"
[backdoor]:               #attacks "\"backdoor\"&#10; undocumented feature created to provide developers with access in case someone locks theirself out of the program"
[botnet]:                 #attacks "\"botnet\"&#10; takes control of a computer, typically in order to conduct a DoS attack"
[keylogger]:              #attacks "\"keylogger\"&#10; logs keystrokes"
[ransomware]:             #attacks "\"ransomware\"&#10; attacker encrypts files on a user's computer and then demands money for their release"
[RAT]:                    #attacks "\"Remote Access Trojan (RAT)\"&#10; program that sends login details to the attacker"
[rootkit]:                #attacks "\"rootkit\"&#10; enables access to a computer or OS which are not otherwise accessible"
[scareware]:              #attacks "\"scareware\"&#10; rogue security software that is, in fact, malware"
[spyware]:                #attacks "\"spyware\"&#10; tracks usage"
[trojan]:                 #attacks "\"trojan\"&#10; creates backdoors in target system"
[virus]:                  #attacks "\"virus\"&#10; program that embeds itself in another program"
[worm]:                   #attacks "\"worm\"&#10; program that replicates itself to spread to other computers, exploiting security weaknesses"

[explicit mode]: #ftps "\"explicit mode\"&#10; client will be able to request encryption"
[implicit mode]: #ftps "\"implicit mode\"&#10; all connections are secure, requiring port 990 to be used as control port"
[Preparation]: # 'Preparation&#10;Different incident response plans are written and kept updated&#10;Neil, Ian. _CompTIA Security+ Certification Guide_: 313'
[Identification]: # 'Identification&#10;Appropriate incident response plan is invoked and personnel notified upon occurrence of an incident&#10;Neil, Ian. _CompTIA Security+ Certification Guide_: 313'
[Containment]: # 'Containment&#10;Preventing incident from spreading further while securing volatile evidence&#10;Neil, Ian. _CompTIA Security+ Certification Guide_: 314'
[Eradication]: # 'Eradication&#10;Destroying the source of the incident&#10;Neil, Ian. _CompTIA Security+ Certification Guide_: 314'
[Recovery]: # 'Recovery&#10;Returning organization to normal operations, within RTO and RPO&#10;Neil, Ian. _CompTIA Security+ Certification Guide_: 314'
[Lessons learned]: # 'Lessons learned&#10;Detective phase where all facts are pulled together to prevent re-occurrence of incident in the future&#10;Neil, Ian. _CompTIA Security+ Certification Guide_: 314'
