...
Server Message Block                              | application-layer or presentation-layer network protocol used for providing shared access to files, printers, serial ports, and other communications between nodes on a network, mostly used by Microsoft Windows; previously known as __Microsoft Windows Network__ before the introduction of __Active Directory__; basis for the __Distributed File System__ | [Wiki](https://en.wikipedia.org/wiki/Server_Message_Block)
