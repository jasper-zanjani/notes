...
2. Compare and contrast types of attacks <br> [Attacks](#attacks)
...
1. Install and configure network components, both hardware- and software-based, to support organizational security <br> Firewall &bull; [IPsec](#ipsec) &bull; False [positive](#risk)/[negative](#risk)
...
2. Given a scenario, implement secure network architecture concepts <br> [DMZ](#network-design-elements)
...
7. Summarize cloud and virtualization concepts <br> [**Cloud deployment models**](n10-007.md#cloud) SaaS PaaS IaaS Private Public Hybrid Community &bull;
#### 4. Application, data, and host security
1. Compare and contrast identity and access management concepts <br> [**Authentication**](#authentication-concepts) Identification Authorization Federation [Single sign-on](#single-sign-on) Transitive trust
2. Given a scenario, install and configure identity and access services <br> [**AAA protocols**](#aaa-protocols) [Kerberos](#kerberos) RADIUS TACACS+ &bull; [CHAP](#chap) PAP MSCHAP &bull; [NTLM](#hashing-algorithms) Shibboleth OAuth OpenID Connect [LDAP](n10-007.md#ldap) [SAML](#authentication-miscellany)
...
1. Compare and contrast basic concepts of cryptography <br> [Symmetric/asymmetric key cryptography](#symmetric-and-asymmetric-key-cryptography) &bull; [Modes of operation](#cipher-modes) [Key stretching](#key-stretching) [Perfect forward secrecy](#forward-secrecy)
...
- **Symmetric key cryptography** also "secret key", "private key", or "shared secret" algorithms, uses the same encryption key to cipher and decipher data [[Dulaney](#sources): 365; [Barrett](#sources): 442]
...
#### Nonrepudiation
**Nonrepudiation** makes it impossible to refute the origin of data [[Barrett](#sources): 447]
...
- **Rivest, Shamir, Adelman (RSA)** asymmetric-key cryptographic algorithm that can use a key of any length and which was released into the public domain in 2000; works by multiplying two large prime numbers and derives a set of numbers through other operations [[Dulaney](#sources): 496; [Barrett](#sources): 461]
  - **Digital Signature Algorithm (DSA)** developed by the NSA in 1991 as an alternative to RSA which is faster in signing but slower in verifying, thus preferable if there are performance issues on the client [[B](#sources)]
- **Diffie-Hellman key exchange (D-H)** also "exponential key agreement", an early asymmetric-key cryptographic algorithm, vulnerable to man-in-the-middle attacks. [[Barrett](#sources): 461]
...
- **Rivest Cipher (RC2, RC5, RC6)** family of interrelated symmetric-key block ciphers [[Barrett](#sources): 459]
...
#### Forward secrecy
**Perfect forward secrecy** process of destroying **session keys** after communication session is complete. A **session key** also "symmetric key", is a randomly generated key to perform both encryption and decryption during a session [[Barrett](#sources): 445-6]\
**Ephemeral key agreement protocol** provides **perfect forward secrecy**, at computational cost [[Barrett](#sources): 446]
- **Diffie-Hellman (DHE)** uses an ephemeral algorithm [[Barrett](#sources): 446]
- **Elliptic Curve Diffie-Hellman (ECDH)** variant of Diffie-Hellman key agreement protocol for ECC using an ephemeral mode of operation [[Barrett](#sources): 446]
...
## PKI
**Digital certificate** block of data signed using CA's private key that allows PKI to be used for identification purposes [[Barrett](#sources): 478]
- **Certificate Authority (CA)** trusted entities that issue certificates and authenticate certificate holders [[Barrett](#sources): 480]
- **Registration authorities (RA)** provide authentication to the CA as to the validity of a client's certificate request and serves as an aggregator of information [[Barrett](#sources): 481]

- **Identity-based encryption (IBE)** third-party server uses something as simple as an email address to identify the user, from which a public key is generated, which can be used for encrypting and decrypting mail [[Barrett](#sources): 478]
- **Key escrow** CA or other entity maintaining a copy of the private key associated with the public key signed by the CA [[Barrett](#sources): 486]
- **M of N control** practice of backing up the public and private keys across multiple system [[Barrett](#sources): 484]
- **PKIX** IETF working group developing Internet standards for PKI based on X.509 certificates [[Barrett](#sources): 476]
- **Pseudo Random Number Generation (PRNG)** produces a sequence of bits that has a random-looking distribution [[Barrett](#sources): 477]
- **Public Key Cryptography Standards (PKCS)** de facto cryptographic message standards, providing a basic and widely accepted framework for the development of PKI solutions; developed and published by RSA Laboratories; comprised of about 15 documents [[Barrett](#sources): 476]
- **X.509** framework for authentication services by a directory [[Barrett](#sources): 477]
...
- **Certificate policy** provides the rules that indicate the purpose and use of an assigned digital certificate and identifies the purpose for which it is to be used [[Barrett](#sources): 481]
- **Certificate practice statement (CPS)** legal document containing general practices followed by the CA in issuing certificates and customer-related information about certificates, responsibilities, and problem management; created and published by a CA for the purpose of conveying information to thsoe depending on the CA's issued certificates [[Barrett](#sources): 482]
...
#### Key management
- **Centralized key management** allows issuing authority to have complete control over the process [[Barrett](#sources): 484]
...
#### Revocation
**Revocation** invalidating a certificate before its expiration date [[Barrett](#sources): 486]
- **Online Certificate Status Protocol (OCSP)** newer protocol used for obtaining the revocation status of an X.509 digital certificate in real time [[Dulaney](#sources): 490; [Barrett](#sources): 487]
- **Certificate revocation list (CRL)** mechanism for distributing certificate revocation information; limited by the need to be constantly updated [[Barrett](#sources): 487]

#### Trust models
- **Single CA** [[Barrett](#sources): 488]
...
- **Identification** occurs when client presents credentials [[Barrett](#sources): 402]
- **Authentication** process of validating an identity [[Dulaney](#sources): 462]; [Barrett](#sources): 402]
- **Authorization** identifying the resources a user can access after authentication; mechanism by which a system grants or revokes the right to access some resource or perform some action [[Dulaney](#sources): 462]; [Barrett](#sources): 402]

- **Federation** assigning an administrative account capable of enumerating local security principals and resources, with the federation system accessible from each domain, allowing accounts in one domain access to resources in other communicating domains [[Barrett](#sources): 416-7]
- **Trust** interoperation assigned directly between two resource pools through administrative actions within each specifically designating the other as a trusted resource pool and allowing enumeration of accounts and available resources, so that access control over any resource can be granted or denied to any account in either domain [[Barrett](#sources): 417]
  - **Transitive trust** trust that can extend beyond two domains to other trusted domains in the forest [[Barrett](#sources): 417; [D](#sources)]
#### CHAP
**Challenge Handshake Authentication Protocol (CHAP)** protocol that challenges a system to verify identity at random intervals by requiring client to send a MD5 hash value back to the service (called the **authenticator**) which verifies the value; improvement on PAP and functions over PPP [[Dulaney](#sources): 170, 468; [Barrett](#sources): 411, 458]
  - **Password Authentication Protocol (PAP)** simple but insecure PPP authentication protocol providing one-way authentication while sending credentials in cleartext, typically used only with older Unix systems that do not support more secure authentication protocols [[Dulaney](#sources): 491; [Barrett](#sources): 411]
  - Microsoft implementations: MS-CHAP and MS-CHAPv2
### Access control
**Access control** process of making resources available to accounts that should have access while limiting that access to only what is required; mechanisms used to filter network traffic to determine who is and is not allowed to access the network and network resources [[Dulaney](#sources): 360; [Barrett](#sources): 404]
- **Least privilege** access control practice wherein a logon is provided only the bare minimum access to resources required to perform its tasks [[Barrett](#sources): 402]
...
- **Implicit deny** access control practice where resource availability is restricted to only those logins explicitly granted access, remaining unavailable even when not explicitly denied access [[Barrett](#sources): 25, 412]

- **Access Control List (ACL)** mechanism for authorizing privileges for accounts; list of trustees assigned to a file or directory, or permitted computers or users in the context of routers or firewalls [[Dulaney](#sources): 462; [Barrett](#sources): 403]
  - Most common privileges include `Read`, `Write`, `Read & Execute`, `Modify`, `Full Control`, and `List Folder Contents`

#### Access control models
- **Mandatory access control (MAC)** also "multilevel access control", form of access control where administrators dictate who can access and modify data, systems, and resources by assigning sensitivity labels or attributes to objects and users [[Dulaney](#sources): 361; [Barrett](#sources): 404]
- **Discretionary Access Control (DAC)** form of access control which is not enforced from the administrator or OS instead controlled by an object's owner [[Dulaney](#sources): 361; [Barrett](#sources): 404]
- **Role-based access control (RBAC)** form of access control where access decisions are based on roles that individual users have within the organization [[Dulaney](#sources): 362; [Barrett](#sources): 406]
- **Rule-based access control (RBAC)** form of access control based on established rules [[Dulaney](#sources): 362; [Barrett](#sources): 405]
- **Attribute-based access control (ABAC)** also "policy-based access control", allows conditional logic to define access control [[C](#sources)]\
...
#### Tokens
One-time password generation standards [[Barrett](#sources): 411]
...
**Smart card** form of "something you have" authentication using a wallet-sized card with an embedded chip that can provide an authenticating cryptographic key to a reader [[Barrett](#sources): 409]
- **Common Access Card (CAC)** DoD smart card
- **Personal Identity Verification (PIV)** smart card used for federal employees and contractors [[Barrett](#sources): 409, 416]

#### AAA protocols
- **Remote Authentication Dial-In User Service (RADIUS)** protocol that enables a single server to become responsible for all remote-access authentication, authorization, and auditing services (UDP ports 1645 and 1646) [[Dulaney](#sources): 363; [Barrett](#sources): 394]
  - **Realm** authentication domain in RADIUS, between several of which RADIUS can forward authentication and authorization requests [[Barrett](#sources): 394]
- **Terminal Access Control Access Control System (TACACS)** security protocol that provides authentication and authorization primarily in Unix networks (TCP port 49) [[Dulaney](#sources): 363; [Barrett](#sources): 394]
  - **TACACS+** Cisco-developed AAA protocol that is similar to RADIUS but uses TCP [[Barrett](#sources): 395]
  - **Extended TACACS (XTACACS)** obsolete Cisco-proprietary version of TACACS [[Barrett](#sources): 398]
- [**Kerberos**](#kerberos) symmetric-key authentication protocol developed by MIT and used in Active Directory to provide an abstracted authentication process (TCP and UDP port 88, previously ports 749 and 750) [[Dulaney](#sources): 364-6, [Lammle](#sources): 402; [Barrett](#sources): 395]
...
- **Ticket-granting ticket (TGT)** time-stamped session key sent by CA to client [[Barrett](#sources): 396]
- **Ticket-granting server (TGS)** destination of TGT sent by client [[Barrett](#sources): 396]
- **Key distribution center (KDC)** destination of client authentication details, rather than target center [[Barrett](#sources): 396]
...
One system creates a challenge code based on a random number, which is sent to the other. The receiving system generates a response code using the challenge code, then creates a challenge code of its own, sending both to the originating system. The originator verifies the response and returns its own response to the challenge code of the second system. Once the second system has verified this last response code, it notifies the originating system and both systems consider themselves mutually authenticated.  [[Barrett](#sources): 396-7]

#### Biometrics
- **False acceptance rate (FAR)** measure of the likelihood that the access system will wrongly accept an access attempt [[Barrett](#sources): 414]
- **False rejection rate (FRR)** measure of the likelihood that the access system fails to recognize an authorized person

#### Single sign-on
**Single sign-on (SSO)** solutions that use a central meta-directory service to reduce user support and authentication complexity [[Barrett](#sources): 412]
- **Shibboleth** ??
- **OAuth** ??
- **OpenID Connect** ??

#### Authentication miscellany
- **Security Assertion Markup Language (SAML)** XML framework for creating and exchanging security information that relies on assertions about identities and needs to use timed sessions [[Barrett](#sources): 398]
- **Service oriented architecture (SOA)** environment in which the client-facing proxy application provides a standard mechanism or "wrapper" for handling specialized logon, authentication, and access control functions [[Barrett](#sources): 412]

Formal security criteria [[Barrett](#sources): 405]
...
B. "Diffie-Hellman, RSA, DSA, ECC and ECDSA – Asymmetric Key Algorithms". [Web](https://www.ssl2buy.com/wiki/diffie-hellman-rsa-dsa-ecc-and-ecdsa-asymmetric-key-algorithms).
C. "Attribute-based access control". [Wiki](https://en.wikipedia.org/wiki/Attribute-based_access_control).
D. "Active Directory". [Wiki](https://en.wikipedia.org/wiki/Active_Directory#Trusting).
