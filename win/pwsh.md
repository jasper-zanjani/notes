...
**G** | **`Guid`** [`n`][New-Guid]
**H** | **`Help`** [`g`][Get-Help] [`Update`][Update-Help]
**I** | **`Item`** [`Copy`][Copy-Item] [`g`][Get-Item] [`n`][New-Item] [`Remove`][Remove-Item]
**L** | **`List`** [`Format`][Format-List] &bull; **`Location`** [`g`][Get-Location] [`s`][Set-Location]
**M** | **`Module`** [`Import`][Import-Module] [`Install`][Install-Module]
**N** | **`NetAdapter`** [`g`][Get-NetAdapter] &bull; **`NetFirewallRule`** [`g`][Get-NetFirewallRule] [`n`][New-NetFirewallRule] [`s`][Set-NetFirewallRule] &bull; **`NetIpAddress`** [`n`][New-NetIpAddress] &bull; **`Null`** [`Out`][Out-Null]
**O** | **`Object`** [`ForEach`][ForEach-Object] [`Select`][Select-Object] [`Where`][Where-Object] &bull; **`Output`** [`Write`][Write-Output]
**P** | **`Partition`** [`g`][Get-Partition] [`n`][New-Partition] [`Remove`][Remove-Partition] &bull; **`PSReadlineOption`** [`g`][Get-PSReadlineOption] [`s`][Set-PSReadlineOption] &bull; **`PSSession`** [`Disconnect`][Disconnect-PSSession] [`Enter`][Enter-PSSession] [`Exit`][Exit-PSSession] [`g`][Get-PSSession] [`n`][New-PSSession]
**S** | **`Service`** [`g`][Get-Service] [`Start`][Start-Service] [`Stop`][Stop-Service] &bull; **`SmbShare`** [`n`][New-SmbShare]
...
**W** | **`WebRequest`** [`Invoke`][Invoke-WebRequest] &bull; **`WindowsFeature`** [`g`][Get-WindowsFeature] [`Install`][Install-WindowsFeature] &bull; **`WindowsOptionalFeature`** [`Enable`][Enable-WindowsOptionalFeature] &bull; **`WMIObject`** [`g`][Get-WMIObject]
...
O365                  | **`DistributionGroupMember`** [`a`][Add-DistributionGroupMember] [`g`][Get-DistributionGroupMember] [`Update`][Update-DistributionGroupMember]
Remote administration | [`Enable-PSRemoting`][Enable-PSRemoting] [`Invoke-Command`](#invoke-command) &bull; **`PSSession`** [`Disconnect`][Disconnect-PSSession] [`Enter`][Enter-PSSession] [`Exit`](#exit-pssession) [`g`](#get-pssession) [`n`][New-PSSession]
...
**E**   | [`echo`][Write-Host]
**F**   | [`find`][Get-ChildItem]
**G**   | [`grep`][Where-Object]
**L**   | [`less`](#out-host "`Out-Host -Paging`") [`ls`][Get-ChildItem]
...
[Select-Object]:                  #filters                                      "`Select-Object` (alias: `select`) &#10; Select object or object properties"
